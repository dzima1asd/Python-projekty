...
def embed_sequence(table, sequence, opal, iryd, size):
    x, y = opal, iryd
    print(f"Ukrywanie sekwencji: {''.join(sequence)}")  # Debug: Wypisujemy sekwencję do ukrycia
    for char in sequence:
        print(f"Wstawiamy '{char}' na pozycję ({x}, {y})")  # Debug: Wypisujemy pozycje, gdzie wstawiamy znak
        table[y * size + x] = char
        x = (x + 1) % size
        if x == 0:
            y = (y + 1) % size
...
def encrypt_to_scyphrogram(pre_encrypted, white_key, black_key, chars):
    ...
    scyphrogram = ""
    # Dzielimy tekst na sekwencje 10 znaków
    ...
    # Upewniamy się, że długość szyfrogramu jest 30000 znaków
...
def decrypt_from_scyphrogram(scyphrogram, white_key, black_key, chars):
    ...
    print(f"Liczba tablic: {len(tables)}")  # Debug: Liczba tablic
    ...
    for i, table_data in enumerate(tables):
        ...
        print(f"Tablica {i}: opal = {opal}, iryd = {iryd}")  # Debug: Wypisanie współrzędnych opal/iryd
        ...
        for _ in range(10):
            if y * table_size + x < len(table):
                ...
                print(f"Znak odszyfrowany: {table[y * table_size + x]} na ({x}, {y})")  # Debug: Wypisanie odszyfrowanego znaku
            else:
                break
            x = (x + 1) % table_size
            if x == 0:
