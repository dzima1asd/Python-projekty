...
def encrypt_to_scyphrogram(pre_encrypted_text, white_key, black_key, chars):
    ...
    for i, seq in enumerate(sequences):
        print(f"Sekwencja do zaszyfrowania: {seq}")  # Wyświetlanie sekwencji
...
# Funkcja deszyfrowania z szyfrogramu
def decrypt_from_scyphrogram(scyphrogram, white_key, black_key, chars):
    ...
    for i, table_data in enumerate(tables):
        ...
        if not sprawdz_wspolrzedne(opal, iryd, table_size):
            print(f"Pomijanie tablicy z powodu błędnych współrzędnych: opal={opal}, iryd={iryd}")
            continue  # Pomijanie niepoprawnych współrzędnych
        ...
        for _ in range(10):  # Maksymalnie 10 znaków
            ...
            if 0 <= x < table_size and 0 <= y < table_size:
                ...
                print(f"Błąd: Indeks poza zakresem przy x={x}, y={y}")
                break
        
        decrypted_sequences.append(''.join(sequence))

        # Wyświetlenie tablicy po deszyfrowaniu
