...
# Funkcja tworząca szyfrogram z tekstu
def encrypt_to_scyphrogram(pre_encrypted, white_key, black_key, chars):
    table_size = int(math.sqrt(len(pre_encrypted)))  # Rozmiar tablicy
    table = generate_table(table_size, chars)  # Generujemy tablicę
    scyphrogram = ""
    
    # Ukrywanie zaszyfrowanego tekstu w tablicy
    for i, char in enumerate(pre_encrypted):
        ...
        embed_sequence(table, pre_encrypted[i:i + 10], opal, iryd, table_size)  # Sekwencja 10 znaków
        scyphrogram += ''.join(table)  # Dodajemy tablicę do szyfrogramu
...
def decrypt_from_scyphrogram(scyphrogram, white_key, black_key, chars):
    ...
    for i, table_data in enumerate(tables):
        ...
        print(''.join(table))
    return ''.join(decrypted_sequences)
print(f"Indeks x: {x}, y: {y}, Rozmiar tablicy: {table_size}")
if x >= table_size or y >= table_size:
    print(f"Przekroczenie rozmiaru tablicy: x={x}, y={y}, Rozmiar={table_size}")
    continue  # Ignoruj sekwencję poza zakresem
